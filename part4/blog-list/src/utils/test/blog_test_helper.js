...
const favoriteBlog = async (blogs) => {
  const mostLikes = await blogs.reduce( (cur_max, blog) => Math.max(cur_max, blog.likes), 0 );
  return blogs.find( blog => blog.likes === mostLikes );
}

const mostBlogs = async (blogs) => {
  const blogAuthors = await blogs
    .reduce( (authors, blog) => {
  ...
  const authorWithMostBlog = await blogAuthors
    .reduce(
...
const mostLikes = async (blogs) => {
  ...
  const authorWithMostLikes = await blogAuthors
    .reduce(
...
const getNewToken = async (api) => {
  const user = {
    name: 'dummy',
    username: `dummy${Math.floor(Math.random()*10**6)}`,
    password: 'dummy'
  }
